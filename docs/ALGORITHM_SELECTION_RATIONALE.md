...
- **Our Data**: 7 months, 8,085 draws, 485K records after feature engineering
...
- **LightGBM**: Higher overfitting risk on 7-month data
...
- **SVM**: Doesn't scale well to 485K records
- **Logistic Regression**: Too simple, can't capture complex interactions
...
2. **Performance will be MODEST** (not near-perfect):
   ...
   - Better than random guessing (baseline ~0.067)
...
- **Cons**: Not a machine learning algorithm (it's statistical simulation)
- **Verdict**: ❌ Doesn't meet assignment criteria
...
3. **ML Pipeline**: Data collection → preprocessing → training → evaluation
...
Not because it can "beat the lottery" (it can't), but because it:
